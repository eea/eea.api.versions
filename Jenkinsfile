pipeline {
  ...
  environment {
        GIT_NAME = "eea.api.versions"
        SONARQUBE_TAGS = "www.eea.europa.eu,www.eea.europa.eu-en,demo-www.eea.europa.eu,biodiversity.europa.eu,demo-ied.eea.europa.eu,www.ied.eea.europa.eu-en"
    }
      ...
      steps {
        parallel(
          ...
          "PEP8": {
            node(label: 'docker') {
              script {
                sh '''docker run -i --rm --name="$BUILD_TAG-pep8" -e GIT_SRC="https://github.com/eea/$GIT_NAME.git" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" -e PARAMS="--exclude=interfaces.py,evolve*.py" eeacms/pep8'''
              }
            }
          },
